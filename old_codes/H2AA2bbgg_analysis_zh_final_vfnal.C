...
//          if (nMuon < 2) continue;
	...
	/*	if (isMu) {
                lepton1vec.SetPtEtaPhiE(fabs(Muon_pt[0]), Muon_eta[0], Muon_phi[0], sqrt(fabs(Muon_pt[0])*fabs(Muon_pt[0]) + 0.105*0.105));
                lepton2vec.SetPtEtaPhiE(fabs(Muon_pt[1]), Muon_eta[1], Muon_phi[1], sqrt(fabs(Muon_pt[1])*fabs(Muon_pt[1]) + 0.105*0.105));
                }     */

		if (isMu) {
                lepton1vec.SetPtEtaPhiE(fabs(Electron_pt[0]), Muon_eta[0], Electron_phi[0], Electron_e[0]);
                lepton2vec.SetPtEtaPhiE(fabs(Electron_pt[1]), Muon_eta[1], Electron_phi[1], Electron_e[1]);
                }
